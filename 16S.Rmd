...
ann_colors <- list(
   batch = c("B1"="#34b9ed",
             ...
             "B6"='#bf4380',
             "B7"='#f0e74c'),
   phenotype = c(`1st`=  "#024cc1",
              `2nd` = "#a100bf",
              `3rd` =  "#76B7B2",
              `4th`=  "#D62728"))
"#34b9ed",'#cc7daa','#ff594c','#FFC266''#4E79A7'
"#ff594c", "#9467BD"
#1d8dcf #ff594c #9467BD #4E79A7 clusters
```
...
save_pheatmap_pdf(p1, filenames,width=70, height = 100)
...
palette <- c("#34b9ed",'#cc7daa','#ff594c','#9467BD','#FFC266')
...
#figure
#dev.off()
ggsave("./figures/permanova_asv.png",figure,dpi=300,height = 10,width=15)
```
#beta and alpha 
...
```{r,fig.width =5,fig.height = 4}
 ...
 palette <- c10
  ...
  p <- ggplot(df_long, aes(x = Sample, y = Count, fill = plot)) +
    ...
    geom_text(aes(label = round(Count,3)), position = position_stack(vjust = 0.5), size = 4) +
    labs(x = "Number of Dirichlet mixture components", y = "Relative abundance top 10 drivers",tag = 'C') +
    theme_linedraw() + labs(x = "Enterotype") +
  theme(
...
palette <- c('#1d8dcf', '#ff594c', '#9467BD', "#bf4380")
plot_list <- list()
...
age_dmm <- create_plot_rel(c('phenotype','best'),c('#1d8dcf', '#ff594c', '#9467BD', "#bf4380"),ps_meta) + 
...
sex_dmm <-create_plot_rel(c('sex','best'),c('#1d8dcf', '#ff594c', '#9467BD', "#bf4380"),ps_meta)

batch_dmm <- create_plot_rel(c('batch','best'),c('#1d8dcf', '#ff594c', '#9467BD',  "#bf4380"),ps_meta) + 
             labs(fill = "Enterotype") +
  theme(
    axis.title.y = element_text(size = 20, color = "black"),
    axis.title.x = element_text(size = 20, color = "black"),
...
p <- p + ( age_dmm/ sex_dmm/batch_dmm) + plot_annotation(tag_levels = "A") & theme(plot.tag = element_text(size=14,face="bold",colour='black') )
p
ggsave('./figures/biochem_distribution_dmm_clusters.png',p,dpi=300,width=15,height = 10)
...
variables <- c("batch", "sex", "age_at_heath_check_10yr", "education", "cancer",
               "kidney_disease", "drinking_level", "lipid_mod_agents_med", 
               "antidiab_med", "dietary_quality_score_cat", 
               "current_smoker", "total_phys_act_index", "ISCO_group")
...
plots <- lapply(variables, function(var) {
  create_plot_rel(c("phenotype", var), colors, ps_meta)
})
...
# pp
# ggsave(plot=pp, path=wd_path, filename="top_drivers.pdf", device='pdf', width = 20, height = 35)
# heatmapdmn
```

```{r, fig.width=15,fig.height=7}
p_top_taxa <- grid.arrange(
  grobs = c(pp),  # Combine pp with age_dmm and batch_dmm
  nrow = 2  # Set the number of rows to 3
)
```
...
# Assuming pp is a list of plots for the first two rows
# Arrange pp plots in two rows and add age_dmm and batch_dmm centered in the third row
library(patchwork)

# Define p_top_taxa and entero layouts separately
p_top_taxa <- ((pp[[1]] + pp[[2]]) /      # First row with plots from pp
               (pp[[3]] + pp[[4]]))       # Second row with more plots from pp
               
entero <- (age_dmm / batch_dmm)           # Stack age_dmm over batch_dmm

# Combine p_top_taxa and entero with desired layout and annotations
final_plot <- (p_top_taxa | entero) + 
              plot_layout(widths = c(3, 1)) +
              plot_annotation(tag_levels = list(c("A",'','','', "B", "C")))& 
  theme(plot.tag = element_text(size = 25)) # Manually set tag levels as A, B, C
...
for (i in seq_along(cols_to_keep)) {
  ...
  plot_list[[cols[2]]] <- ggplot(ps_meta, aes(x = !!sym(cols[1]), y = !!sym(cols[2]), fill = !!sym(cols[1]))) +
    ...
    geom_boxplot(width = 0.5, alpha = 0.75, position = position_dodge(0.9)) +
    geom_jitter(size = 1.5, alpha = 0.5, position = position_jitterdodge(jitter.width = 0.1, dodge.width = 0.9)) +
    scale_fill_manual(values = c("#024cc1","#76B7B2","#a100bf","#D62728")) +
...
ps_obj <- microbiome::transform(ps_obj, trans)
palette <- c('#1d8dcf', '#ff594c', '#9467BD',  "#bf4380")
ps_obj@sam_data$enterotype <- ps_obj@sam_data$best
beta_dmm <- create_ordination_plots(ps_obj, method = "PCoA", distance_method = distance_method , group = "enterotype", size = 15,palette=palette )
 ...
 palette <- c10
  ...
  p <- ggplot(df_long, aes(x = Sample, y = Count, fill = plot)) +
    ...
    scale_fill_manual("legend", values = palette, name = 'ASV') +
    geom_text(aes(label = round(Count,3)), position = position_stack(vjust = 0.5), size = 4) +
    labs(x = "Number of Dirichlet mixture components", y = "Relative abundance top 10 drivers",tag = 'B') + labs(x = "Enterotype") +
    theme(axis.text.x = element_text(hjust = 1,size = 16,colour = 'black'),
           # Background elements for full white background
  panel.background = element_rect(fill = "white"),
  plot.background = element_rect(fill = "white"),
  axis.line = element_line(colour = 'black'),
          plot.tag=element_text(size=16,face="bold",colour = 'black'),
          axis.text.y = element_text(size = 16,colour = 'black'),
   axis.title.y = element_text(size = 16,colour = 'black'),
   axis.title.x = element_text(size = 16,colour = 'black'),
          legend.text =  element_text(size = 16, family = "Fira Sans"),
          legend.title = element_text(size = 16,family = "Fira Sans",face = "bold"))
...
p_dmm <- ggarrange(
  ggarrange(beta_dmm, labels = 'A'),  # Set label "a" for beta_dmm
  p,                 # Set label "b" for p
  ncol = 2
)
p_dmm
ggsave("./figures/dmm_beta.png",p_dmm,dpi=300,height = 15,width=25)
