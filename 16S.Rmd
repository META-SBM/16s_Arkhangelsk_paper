...
library(showtext)
#font_add_google("Times New Roman", "times")
#octobiom::setup_plot_fonts("Times New Roman")
ps_obj <- octobiom::prepare_phyloseq(ps,transformation = 'compositional',detection = 10,prev = 0.05,taxonomic_level = 'Genus',
                                     first = 'filter')
...
features <- c('phenotype')#,'sex',
              #"smoking_status","YEAR" )
color_palette <- octobiom::get_palette("set_10_4")
plot_alpha_shannon <- purrr::map(features, ~ {
  tryCatch({
    octobiom::create_alpha_plots(ps_obj, col= .x, colors = color_palette,measure = c("Shannon"),size=18,text_size=8,bracket_nudge_y = 0.2)
  }, error = function(e) {
    message(paste("Error in method:", .x))
    message(e$message)  # Print the error message
    return(NULL)  # Return NULL for this iteration
...
plot_alpha_simpson <- purrr::map(features, ~ {
  tryCatch({
    octobiom::create_alpha_plots(ps_obj, col= .x, colors = color_palette,measure = c('Simpson'),size=18,text_size=8,bracket_nudge_y = 0.2)
  }, error = function(e) {
    message(paste("Error in method:", .x))
    message(e$message)  # Print the error message
    return(NULL)  # Return NULL for this iteration
...
plot_alpha_list_chao <- purrr::map(features, ~ {
  tryCatch({
    octobiom::create_alpha_plots(ps_obj, col= .x, colors = color_palette,measure = c("Chao1"),size=18,text_size=8,bracket_nudge_y = 0.1)
  }, error = function(e) {
    message(paste("Error in method:", .x))
    message(e$message)  # Print the error message
    return(NULL)  # Return NULL for this iteration
...
p <- ggpubr::ggarrange(plot_alpha_shannon[[1]]$plots,plot_alpha_simpson[[1]]$plotsplot_alpha_list_chao[[1]]$plots)
p
p <- (plot_alpha_shannon[[1]]$plots | plot_alpha_simpson[[1]]$plots | plot_alpha_list_chao[[1]]$plots) + 
  theme(plot.margin = unit(c(0.5, 0.5, 0, 0), "cm"))
p
ggsave('./figures/alpha_phenotype_genus.png', plot = p, width = 12, height = 6, units = "in", dpi = 300)
```
#Сети 
```{r,fig.height=8,fig.width=8}
