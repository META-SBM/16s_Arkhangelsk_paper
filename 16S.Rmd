...
ggsave("./figures/alpha_beta.png",figure,dpi=300,height = 10,width=14)
```
# dmm
```{r,fig.width=10}
ps_obj <- ps
...
png("./figures/dmm.png",width = 30, height = 32, units = "cm",res = 300)
grid.newpage()
grid.draw(plots_dmm)
dev.off()
```
...
pdf("./figures/biochem_distribution_dmm_clusters.pdf",width = 30, height = 20)
...
visualize_network_analysis <- function(network_analysis_results) {
  for (combination_name in names(network_analysis_results)) {
    ...
    # Optionally, you can also save the plots
    pdf_filename <- paste0("./figures/network_plot__", combination_name, ".pdf")
