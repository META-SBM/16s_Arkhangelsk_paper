...
ps_obj <- octobiom::prepare_phyloseq(ps,transformation = 'NONE',detection = 10,prev = 0.1,taxonomic_level = 'ASV',
                                     first = 'filter')
#otu_table <- read.table("otu_table_filtered.tsv", header = TRUE, row.names = 1, sep = ",")
#biom_obj <- make_biom(data = as.matrix(otu_table))
#write_biom(biom_obj, "otu_table_filtered.tsv")
t(ps_obj@otu_table)

write.table(t(ps_obj@otu_table),'otu_table_filtered.txt',sep = "\t",
            row.names = TRUE)
write.csv(ps@tax_table,'tax_table.tsv')
...
df <- read.csv('~/DAVID_WGS/david_all_database_2017.csv')
...
cor_data <- df %>%
  group_by(d_sex) %>%
  summarise(
    cor = cor.test(d_age_b, hcd_bmi, method = "pearson")$estimate,
    p_value = cor.test(d_age_b, hcd_bmi, method = "pearson")$p.value,
    label = paste0("r = ", round(cor, 2), "\np = ", round(p_value, 3))
  )
ggplot(df, aes(x =  d_age_b, y = hcd_bmi)) +
  geom_smooth()+
  # Линии и точки с цветом по возрастной группе
  ...
  labs(
    x = "age",
    y = "BMI",
    #subtitle = "ΔShannon = Shannon(2022) - Shannon(2017)"
  ) +
  theme_minimal() +
  theme(
  ...
  facet_grid(
    cols = vars(d_sex),  # Увеличение/уменьшение
    #cols = vars(YEAR),               # Пол
    scales = "free_y",              # Свободная шкала Y для каждого фасета
  )+
  geom_text(
    data = cor_data,
...
taxas <- core_members(ps_obj, detection = det, prevalence = prev)
ps_obj <- prune_taxa(taxas, ps_obj)
ps_obj_species <- speedyseq::tax_glom(ps_obj,taxrank = level, NArm=T )

# 1. Рассчитать индекс Шеннона
data_alpha <- estimate_richness(ps_obj, measures = "Shannon")
meta <- as(ps@sam_data,'data.frame')
meta$age_at_heath_check_10yr
plot_data <- meta %>%
  select(ID_KYH,age_at_heath_check,sex,BMI,age_at_heath_check_10yr,smoking_status) %>%
  mutate(
    ID_KYH = factor(ID_KYH),
    sex=as.factor(sex),
    smoking_status = as.factor(smoking_status)
  ) %>%
  arrange(ID_KYH)

plot_data <- merge(plot_data,data_alpha,by='row.names')
plot_data <- plot_data %>%
  mutate(Shannon = as.numeric(Shannon),
         age_at_heath_check = as.numeric(age_at_heath_check),
         BMI = as.numeric(BMI))


shannon <- ggplot(plot_data, aes(x = age_at_heath_check, y = Shannon)) +
  geom_smooth()+
  geom_point( shape = 21, size = 3, color = "black", stroke = 0.5,alpha = 0.4) +
  ...
  facet_grid(
    rows = vars(sex),  # Увеличение/уменьшение
    #cols = vars(YEAR),               # Пол
    scales = "free_y",              # Свободная шкала Y для каждого фасета
  ) 

bmi <- ggplot(plot_data, aes(x = age_at_heath_check, y = BMI)) +
  geom_smooth()+
  geom_point( shape = 21, size = 3, color = "black", stroke = 0.5,alpha = 0.4) +
...
meta$mean_height_cm <- as.numeric(as.character(meta$mean_height_cm))
meta$weight_tanita <- as.numeric(as.character(meta$weight_tanita))
meta$bmi <- meta$weight_tanita/(meta$mean_height_cm/100)**2
...
feature_colors <- c(
  "age_at_heath_check" = "#FF8400",
  "bmi" = "#3C096C",
  'hs.CRP_mg.L' = "#FF0A54",
  'HbA1C_.'="#2D00F7",
  'HDL_cholesterol_mmol.L' ="#029191",
  'triglycerides_mmol.L' ="#08415C"
)
ps_obj@sam_data$bmi <- as.numeric(ps_obj@sam_data$bmi)
ps_obj@sam_data$age_at_heath_check <- as.numeric(ps_obj@sam_data$age_at_heath_check)
...
network_correlation_results <- octobiom::build_multi_correlation_network(
  ps_obj,
  features = c("age_at_heath_check", "bmi",'hs.CRP_mg.L','HbA1C_.','HDL_cholesterol_mmol.L','triglycerides_mmol.L'),
  tax_rank = "Genus",
  cor_method = 'pearson',
...
ps_obj@tax_table['Genus' == 'UCG-005',]
ps_obj@tax_table %>%
  as.data.frame()%>%
  filter(Genus == 'UCG-005')
```
#Dif analysis
```{r,fig.height=8}
...
#metadata
```{r}
df <- read.csv('/home/kuzmichenko_pa/updated_meta_16s.csv',sep=',')
df[,c('hs.CRP_mg.L','new_id')]
df$hs.CRP_mg.L
```
