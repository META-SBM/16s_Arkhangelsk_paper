...
results <- process_maaslin(
  ...
  reference = c("phenotype, 1st","batch,B1"),
  ncores = 1,
  output_dir = '/home/kuzmichenko_pa/maaslin_results_16s_pathway/'
)

results_csv <-  read.csv('/home/kuzmichenko_pa/maaslin_results_16s_pathway/results.csv')
results_csv 
...
```{r}
library(biomformat)
seqtab <- read.csv('/mnt/disk1/RUNS/galeeva/picrust_arhan/seqtab.tsv',sep='\t',row.names =1 )
colnames(seqtab) <- gsub('X','',colnames(seqtab))
taxa <- readRDS('/mnt/disk1/RUNS/galeeva/picrust_arhan/taxa.rds')
taxa <- as(taxa,'matrix')
OTU = otu_table(seqtab, taxa_are_rows = TRUE)
TAX = phyloseq::tax_table(taxa)
samples <- ps_obj@sam_data
ps_biom <- phyloseq(OTU,TAX, samples)
ps_biom
```
