...
#ps_obj@sam_data$Registry_hosp_group1 <- factor(ps_obj@sam_data$Registry_hosp_group,
#                                               levels = c("0", "1", "2"),
#                                               labels = c("not-hospital", "hospital", "not-hospital"))
ps_obj
```
...
contrasts <- list(c("Registry_hosp_group1", 'not-hospital', 'hospital'))
contrasts <- list(c('Lean','Overweight','Obesity'))
diagdds = phyloseq_to_deseq2(ps_obj ,~ bmi_group)
...
names(sigtabs_list) <- c("not-hospital_vs_hospital")
...
combined_df <- combined_df %>%
  dplyr::filter((log2FoldChange < 20) & (log2FoldChange > -20) )
# Example ggbarplot
size <-10
ggbarplot(combined_df, x = "Species", y = 'coef',
          fill = 'Phylum',
          palette =colors,
...
contrasts <- list(c("bmi_group", 'Lean', 'Overweight'), c("bmi_group", 'Lean', 'Obesity'), c("bmi_group", 'Overweight', 'Obesity'))
diagdds = phyloseq_to_deseq2(ps_obj ,~ bmi_group)
diagdds <- DESeq(diagdds, test="Wald", fitType="parametric", sfType = "poscounts")
...
names(sigtabs_list) <- c("Lean_vs_Overweight", "Lean_vs_Obesity","Overweight_vs_Obesity")
# Add a 'comparison' column to each data frame in the list with the corresponding list name
sigtabs_list <- lapply(names(sigtabs_list), function(name) {
  df <- sigtabs_list[[name]]
  df$comparison <- name
...
combined_df <- combined_df %>%
  dplyr::filter((log2FoldChange < 20) & (log2FoldChange > -20) )
table_from_file$Pathway <- rownames(table_from_file)

combined_df <- combined_df %>%
  left_join(table_from_file,by='Pathway')

# Example ggbarplot
p <- ggbarplot(combined_df, x = "taxa", y = 'coef',
          fill = 'Genus',
          palette =c25,
          sort.by.groups = FALSE,  # Don't sort inside each group
          x.text.angle = 90,
...
p
unique_genera <- unique(combined_df$Genus)

# Итерация по каждому Genus
for (genus in unique_genera) {
  # Фильтруем данные для текущего Genus
  #genus <- 'Alistipes'
  genus_data <- combined_df %>% filter(Genus == genus)
  #genus_data <- combined_df %>% filter(Genus == 'Alistipes')
  #genus_data <- genus_data %>%
  #filter(!is.na(definition))
  # Создаем график для текущего Genus
  p <- ggbarplot(genus_data, x = "taxa", y = 'coef',
                 fill = 'group',
                 palette = c25,
                 sort.by.groups = FALSE,  # Don't sort inside each group
                 x.text.angle = 90,
    ...
    ggtitle(paste("График для Genus:", genus)) # Добавляем заголовок
  p
  # Сохраняем график в файл
  ggsave(paste('~/DAVID_WGS/different_figures/ko_humann/',genus,'.pdf'), plot = p, width = 20, height = 20)
}
p
ggsave('~/DAVID_WGS/different_figures/da_ko_taxa.pdf',p,height=100,width=25,limitsize = FALSE)

```
#Нарисуем для Alistipes
...
combined_df_filter <- combined_df %>%
  dplyr::filter(Species == "Alistipes_onderdonkii")
  #dplyr::filter(Genus == "Methanobrevibacter")
# Example ggbarplot
combined_df_filter <- combined_df %>%
  dplyr::filter(Pathway == "K06221")
ggbarplot(combined_df_filter, x = "taxa", y = 'coef',
          fill = 'Genus',
          palette =c25,
          sort.by.groups = FALSE,  # Don't sort inside each group
          x.text.angle = 90,
          lab.size = 5,
          rotate = TRUE,
          label = combined_df_filter$signif, 
          width = 0.4,
          position = position_dodge(0.5),
          ylab = "log2foldchange",  # Adjusted y-axis label
          ggtheme = theme_bw()) + 
...
library(org.Hs.eg.db)                   # Replace with your organism's database
library(enrichplot)

gene_list <- combined_df_filter$log2FoldChange
names(gene_list) <- combined_df_filter$taxa
